...
MafiaEvents.LifeEvents = {
	...
	-- CRITICAL FIX #260: Made FBI offer a one-time event
	{
		...
		{
			text = "Try to flee the country ($100K)",
			...
			eligibility = function(state) return (state.Money or 0) >= 100000, "💸 Need $100K to flee" end,
		},
		},
	},
	...
	-- CRITICAL FIX #265: Added cooldown to promotion_ceremony
	{
		...
		{
			text = "Accept and celebrate lavishly ($10K)",
			...
			eligibility = function(state) return (state.Money or 0) >= 10000, "💸 Need $10K for celebration" end,
		},
			{
	...
	},
	{
		...
		choices = {
		{
			text = "Refuse to talk, lawyer up ($50K)",
			effects = { Happiness = -10, Money = -50000 },
			mafiaEffect = { respect = 20, loyalty = 10 },
			setFlags = { lawyered_up = true },
			feed = "You said nothing. The family is pleased.",
			eligibility = function(state) return (state.Money or 0) >= 50000, "💸 Need $50K for lawyers" end,
		},
			{
		...
		{
			text = "Skip town temporarily ($30K)",
			...
			eligibility = function(state) return (state.Money or 0) >= 30000, "💸 Need $30K to skip town" end,
		},
		},
	},
	{
	...
	-- User complaint: "WHEN IM IN PRISON AND A MAFIA MEMBER THE SAME LIKE 4 LIFEEVENTS SPAM BRUH"
	{
		...
		{
			text = "Cultivate guards on the payroll ($20K)",
			effects = { Happiness = 5, Money = -20000 },
			mafiaEffect = { respect = 15, heat = -5 },
			setFlags = { has_prison_guards = true },
			feed = "A few well-placed bribes make life easier.",
			eligibility = function(state) return (state.Money or 0) >= 20000, "💸 Need $20K for bribes" end,
		},
		},
	},
	...
	-- MONEY DROP GONE WRONG
	{
		...
		{
			text = "Cover the loss from your own pocket ($50K)",
			effects = { Happiness = -10, Money = -50000 },
			mafiaEffect = { respect = 20, loyalty = 30 },
			setFlags = { takes_responsibility = true },
			feed = "Expensive, but your reputation is intact.",
			eligibility = function(state) return (state.Money or 0) >= 50000, "💸 Need $50K to cover loss" end,
		},
			{
