...
-- CRITICAL FIX #40: Create early reference to MobFamilies for use throughout the file
-- This prevents "attempt to index nil with 'italian'" errors when MobFamilies is used
-- before its local definition later in the file
local MobFamilies = MobSystemModule.Families
...
function LifeBackend:processMobYearly(state)
	...
	local familyEmoji = state.MobState.familyEmoji or "🔫"
	local rankName = state.MobState.rankName or "member"
	...
	-- Random yearly mob events - now creates actual event definitions!
	local roll = RANDOM:NextInteger(1, 100)

	if roll <= 5 then
		-- Family war (5% chance)
		debugPrint("Mob family war event for", state.Name or "player")
		table.insert(mobEvents, {
			id = "mob_war_" .. os.time(),
			title = familyEmoji .. " Family War",
			emoji = "⚔️",
			text = string.format("A rival organization has declared war on %s! As a %s, you must help defend the family.", familyName, rankName),
			question = "What do you do?",
			category = "mob",
			source = "mobsystem",
			choices = {
				{ text = "Fight for the family", deltas = { Health = -10 }, mobEffect = { respect = 30, heat = 20 }, feed = "fought bravely in the mob war." },
				{ text = "Stay out of it", deltas = { Happiness = -5 }, mobEffect = { loyalty = -20 }, feed = "avoided the mob war." },
				{ text = "Use it to rise up", deltas = { Happiness = 5 }, mobEffect = { respect = 50, heat = 30, risk = 40 }, feed = "used the chaos to advance your position." },
			},
		})
	elseif roll <= 10 and (state.MobState.heat or 0) > 50 then
		-- Police crackdown if heat is high (5% chance)
		debugPrint("Police crackdown on mob, heat:", state.MobState.heat)
		table.insert(mobEvents, {
			id = "mob_crackdown_" .. os.time(),
			title = "🚨 Police Crackdown",
			emoji = "🚔",
			text = string.format("The cops are cracking down on %s! They're looking for anyone connected to organized crime. Your heat level is dangerously high.", familyName),
			question = "How do you handle this?",
			category = "mob",
			source = "mobsystem",
			choices = {
				{ text = "Lay low for a while", deltas = { Happiness = -3 }, mobEffect = { heat = -15 }, feed = "laid low to avoid the cops." },
				{ text = "Bribe the police", money = -5000, mobEffect = { heat = -30 }, feed = "bribed the police to look the other way." },
				{ text = "Continue operations", deltas = { Happiness = 2 }, mobEffect = { heat = 25, respect = 10 }, feed = "continued mob activities despite the crackdown." },
			},
		})
	elseif roll <= 15 then
		-- Loyalty test (5% chance)
		debugPrint("Mob loyalty test for", state.Name or "player")
		table.insert(mobEvents, {
			id = "mob_loyalty_" .. os.time(),
			title = familyEmoji .. " Loyalty Test",
			emoji = "🤫",
			text = string.format("The boss of %s wants to test your loyalty. Someone in the family might be a snitch, and they want you to prove you're trustworthy.", familyName),
			question = "How do you prove yourself?",
			category = "mob",
			source = "mobsystem",
			choices = {
				{ text = "Complete a dangerous task", deltas = { Health = -5, Happiness = 5 }, mobEffect = { loyalty = 20, respect = 15 }, feed = "proved loyalty with a dangerous task." },
				{ text = "Provide information on a rival", mobEffect = { loyalty = 15, heat = 10 }, feed = "provided intel on a rival organization." },
				{ text = "Refuse the test", deltas = { Happiness = -5 }, mobEffect = { loyalty = -25 }, feed = "refused the loyalty test." },
			},
		})
	elseif roll <= 25 then
		-- Bonus from family earnings (10% chance - increased for more action)
		local bonus = RANDOM:NextInteger(500, 3000) * (state.MobState.rankLevel or 1)
		self:addMoney(state, bonus)
		debugPrint("Mob family bonus: $", bonus)
		-- This is passive - add to year log instead of event
		self:logYearEvent(state, "mob",
			string.format("%s You received $%s from %s's operations.", familyEmoji, formatMoney(bonus), familyName), familyEmoji)
	elseif roll <= 30 then
		-- Rank-up opportunity if close to next rank (5% chance)
		...
		if family then
			...
			if nextRankIdx <= #family.ranks then
				...
				local respectNeeded = nextRankDef.respect
				local currentRespect = state.MobState.respect or 0

				-- If they're within 50% of the next rank, offer an opportunity
				if currentRespect >= respectNeeded * 0.5 then
					table.insert(mobEvents, {
						id = "mob_promotion_" .. os.time(),
						title = familyEmoji .. " Promotion Opportunity",
						emoji = "📈",
						text = string.format("Word around %s is that there might be an opening for %s. The current %s is... no longer with us. You could make a move.", familyName, nextRankDef.name, nextRankDef.name),
						question = "Do you seize this opportunity?",
						category = "mob",
						source = "mobsystem",
						choices = {
							{ text = "Make your move", deltas = { Happiness = 5 }, mobEffect = { respect = 40, heat = 15 }, feed = "made a move to advance in the family." },
							{ text = "Wait for the right time", deltas = {}, mobEffect = { loyalty = 5 }, feed = "patiently waited for advancement." },
							{ text = "Decline - too risky", deltas = { Happiness = -2 }, mobEffect = { loyalty = -5 }, feed = "declined a promotion opportunity." },
						},
					})
				end
			end
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- CRITICAL FIX #47: Check for mob rank promotions based on respect
-- When players earn enough respect, they should automatically be promoted
-- ═══════════════════════════════════════════════════════════════════════════════
function LifeBackend:checkMobRankUp(state)
	if not state.MobState or not state.MobState.inMob then
		return false
	end

	local mob = state.MobState
	local family = MobFamilies[mob.familyId]
	if not family then
		return false
	end

	local currentRankIdx = mob.rankLevel or 1
	local nextRankIdx = currentRankIdx + 1

	-- Can't promote past the top rank
	if nextRankIdx > #family.ranks then
		return false
	end

	local nextRank = family.ranks[nextRankIdx]
	local currentRespect = mob.respect or 0

	-- Check if player has enough respect for promotion
	if currentRespect >= nextRank.respect then
		-- Promote!
		mob.rankLevel = nextRankIdx
		mob.rankName = nextRank.name
		mob.rankEmoji = nextRank.emoji or mob.rankEmoji

		-- Log the promotion
		local familyEmoji = mob.familyEmoji or "🔫"
		self:logYearEvent(state, "mob",
			string.format("%s You've been promoted to %s in %s!", familyEmoji, nextRank.name, mob.familyName or "the family"),
			familyEmoji)

		debugPrint(string.format("Mob rank up: %s -> %s (respect: %d)", family.ranks[currentRankIdx].name, nextRank.name, currentRespect))
		return true
	end
...
function LifeBackend:handleAgeUp(player)
	...
	-- CRITICAL FIX #45: processMobYearly is now called during queue-building so events can be displayed
	-- The function handles heat decay, family events, AND returns events for the queue
	...
	-- CRITICAL FIX #45: Process mob yearly and get events for the queue
	-- This handles heat decay, family bonuses, AND returns dramatic events (wars, crackdowns, loyalty tests)
	local mobEvents = self:processMobYearly(state)
	if mobEvents and #mobEvents > 0 then
		for _, mobEvent in ipairs(mobEvents) do
			mobEvent.source = "mob"
			mobEvent.priority = 100 -- High priority - show mob events first
			table.insert(queue, mobEvent)
		end
	end
...
function LifeBackend:resolvePendingEvent(player, eventId, choiceIndex)
	...
	if choice.effects or eventDef.source == "lifeevents" or eventDef.source == "stage" then
		...
		-- ═══════════════════════════════════════════════════════════════════════════════
		-- CRITICAL FIX #46: Handle mobEffect for organized crime choices
		-- This applies respect, heat, loyalty, and risk changes from mob events
		-- ═══════════════════════════════════════════════════════════════════════════════
		if choice.mobEffect and state.MobState and state.MobState.inMob then
			local mob = state.MobState
			local effect = choice.mobEffect

			-- Apply respect changes
			if effect.respect then
				mob.respect = (mob.respect or 0) + effect.respect
				debugPrint(string.format("Mob respect changed: %+d (now %d)", effect.respect, mob.respect))
			end

			-- Apply heat changes
			if effect.heat then
				mob.heat = math.max(0, (mob.heat or 0) + effect.heat)
				debugPrint(string.format("Mob heat changed: %+d (now %d)", effect.heat, mob.heat))
			end

			-- Apply loyalty changes
			if effect.loyalty then
				mob.loyalty = clamp((mob.loyalty or 50) + effect.loyalty)
				debugPrint(string.format("Mob loyalty changed: %+d (now %d)", effect.loyalty, mob.loyalty))
			end

			-- Apply money from mob operations
			if choice.money then
				if choice.money > 0 then
					self:addMoney(state, choice.money)
					effectsSummary = effectsSummary or {}
					effectsSummary.Money = (effectsSummary.Money or 0) + choice.money
				elseif choice.money < 0 and state.Money >= -choice.money then
					self:addMoney(state, choice.money)
					effectsSummary = effectsSummary or {}
					effectsSummary.Money = (effectsSummary.Money or 0) + choice.money
				end
			...
			-- Handle risk - chance of getting caught or hurt
			if effect.risk and effect.risk > 0 then
				local riskRoll = RANDOM:NextInteger(1, 100)
				if riskRoll <= effect.risk then
					-- Bad outcome - could get caught or injured
					local badOutcome = RANDOM:NextInteger(1, 3)
					if badOutcome == 1 then
						-- Caught by police
						state.InJail = true
						state.JailYearsLeft = RANDOM:NextInteger(1, 5)
						self:logYearEvent(state, "crime", "🚔 Your mob activities caught up with you - arrested!", "🚔")
						debugPrint("Player arrested due to mob risk roll")
					elseif badOutcome == 2 then
						-- Injured
						local damage = RANDOM:NextInteger(10, 25)
						state.Stats.Health = clamp((state.Stats.Health or 100) - damage)
						state.Health = state.Stats.Health
						self:logYearEvent(state, "crime", "🏥 You were injured during mob operations.", "🏥")
						debugPrint("Player injured due to mob risk roll")
					else
						-- Heat spike
						mob.heat = (mob.heat or 0) + 30
						self:logYearEvent(state, "crime", "🔥 Your actions drew unwanted attention.", "🔥")
						debugPrint("Heat spike due to mob risk roll")
					end
				end
			end

			-- Check for rank up based on respect
			self:checkMobRankUp(state)
		end
...
-- CRITICAL FIX #43: Removed duplicate MobFamilies definition
-- MobFamilies is now defined early in the file (line ~36) using MobSystemModule.Families
-- This prevents variable shadowing and ensures consistent data across the codebase
...
-- CRITICAL FIX #41: Added explicit TEST_MODE flag
-- Set to true during development/testing to allow access to premium features without gamepasses
-- Set to false for production to require actual gamepass ownership
local GAMEPASS_TEST_MODE = true -- Set to false for production!
...
function LifeBackend:checkGamepassOwnership(player, gamepassKey)
	...
	-- CRITICAL FIX #41: Check TEST_MODE flag for development
	if not gamepassId or gamepassId == 0 then
		if GAMEPASS_TEST_MODE then
			-- In test mode, allow access without configured IDs
			debugPrint(string.format("⚠️ TEST MODE: Gamepass '%s' granted for testing (no ID configured).", gamepassKey))
			return true
		else
			-- In production mode, deny access if not configured
			debugPrint(string.format("⚠️ Gamepass '%s' has no ID configured. Access DENIED (TEST_MODE=false).", gamepassKey))
			return false
		end
	end
